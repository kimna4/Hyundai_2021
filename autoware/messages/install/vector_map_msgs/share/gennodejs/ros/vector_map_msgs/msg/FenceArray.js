...
class FenceArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.data.forEach((val) => {
      bufferOffset = Fence.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.data[i] = Fence.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static datatype() {
    ...
    return 'vector_map_msgs/FenceArray';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new FenceArray(null);
    ...
    if (msg.data !== undefined) {
      ...
      for (let i = 0; i < resolved.data.length; ++i) {
        resolved.data[i] = Fence.Resolve(msg.data[i]);
      }
    }
    else {
      resolved.data = []
