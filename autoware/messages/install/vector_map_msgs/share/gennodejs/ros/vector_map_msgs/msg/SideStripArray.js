...
class SideStripArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.data.forEach((val) => {
      bufferOffset = SideStrip.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new SideStripArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.data[i] = SideStrip.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new SideStripArray(null);
    ...
    if (msg.data !== undefined) {
      ...
      for (let i = 0; i < resolved.data.length; ++i) {
        resolved.data[i] = SideStrip.Resolve(msg.data[i]);
      }
    }
    else {
      resolved.data = []
