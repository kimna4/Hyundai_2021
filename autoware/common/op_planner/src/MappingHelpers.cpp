    ...
    for(unsigned int i =0; i < map.roadSegments.at(rs).Lanes.size(); i++)
    {
      ...
      for(unsigned int i2 =0; i2 < map.roadSegments.at(rs).Lanes.size(); i2++)
      {
        ...
        for(unsigned int p=0; p < pL->points.size(); p++)
        {
          ...
          if(!info.bAfter && !info.bBefore && fabs(info.perp_distance) > 1.2 && fabs(info.perp_distance) < 4.2 && UtilityH::AngleBetweenTwoAnglesPositive(info.perp_point.pos.a, pWP->pos.a) < 0.06)
          {
