...
/*
    ...
    float steering_tune = (direction * distance* pp_.getCurrentVelocity() * 3.6 * 0.001);
    //float steering_tune = (direction * pp_.getCurrentVelocity() * 3.6 * 0.01 * fabs(cur_target_angle));
    //ROS_WARN_STREAM("TUNE : " << steering_tune << std::endl);
    if(fabs(steering_tune) < 0.01) {
      steering_tune = 0;
    }
...
/*
    if(fabs(steering_tune) < 0.03) {
      steering_tune = 0;
    }
*/
/*
    else if(fabs(steering_tune) > 0.523599) {
      steering_tune = 0.523599 * direction;
    }
*/
    ...
    Stanley stanley;
    double heading_error = stanley.get_heading_error(pp_.getCurrentWaypoints(), cur_yaw);
    if(fabs(heading_error) < 0.01) {
      heading_error = 0;
    }
    
    if(std::isnan(heading_error) == 0) {
    //float direction_tune = tan(atan(wheel_base_ * kappa) + steering_tune) / wheel_base_;
      ROS_WARN_STREAM("err1 : "<< heading_error << std::endl); 
      steering_tune = steering_tune + heading_error + atan(wheel_base_ * kappa);

      if(steering_tune > 1.2) {
        steering_tune = 1.2;
      }
      ...
      float direction_tune = tan(steering_tune) / wheel_base_;



      ROS_WARN_STREAM("err2 : "<< heading_error << std::endl); 
