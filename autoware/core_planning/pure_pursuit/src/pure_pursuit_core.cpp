...
void PurePursuitNode::run()
{
  ...
  while (ros::ok())
  {
    ...
    if(way_x < 232 && way_x > 226 && way_y < -79 && way_y > -84) {
      kappa = tan(0) / wheel_base_;
    }
/*
    if(way_x < 218 && way_x > 213 && way_y < -88 && way_y > -89) {
      kappa = tan(atan(wheel_base_ * kappa) * 2.5) / wheel_base_; 
      //ROS_WARN_STREAM("IN " << std::endl);       
    }

    if(way_x < 213 && way_x > 203 && way_y < -81 && way_y > -89) {
      ROS_WARN_STREAM("IN : " << atan(wheel_base_ * kappa) * (3 + stanley_factor_)  << std::endl);       
      kappa = tan(atan(wheel_base_ * kappa) * (3 + stanley_factor_)) / wheel_base_; 
    }
*/
    if(way_x < 206 && way_x > 204 && way_y < -82 && way_y > -85) {
      kappa = tan(atan(wheel_base_ * kappa) * (4 + stanley_factor_)) / wheel_base_; 
      ROS_WARN_STREAM("IN " << std::endl);       
    }
