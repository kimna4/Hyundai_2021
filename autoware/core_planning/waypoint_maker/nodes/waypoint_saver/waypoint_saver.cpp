...
private:
  ...
  void poseCallback(const nav_msgs::Odometry &odom_msg) const;
...
WaypointSaver::WaypointSaver() : private_nh_("~")
{
  ...
  // subscriber
  pose_sub_ = nh_.subscribe("Odometry", 50, &WaypointSaver::poseCallback, this);
...
*/
void WaypointSaver::poseCallback(const nav_msgs::Odometry &odom_msg) const
{
  outputProcessing(odom_msg.pose.pose, 0);
}
