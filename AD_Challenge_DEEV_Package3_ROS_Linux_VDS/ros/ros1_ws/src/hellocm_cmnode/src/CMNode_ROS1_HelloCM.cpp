...
static struct {
    ...
    struct {
        ...
        struct {
			...
			tRosIF_TpcPub<sensor_msgs::NavSatFix> GPS_Out;
            tRosIF_TpcPub<sensor_msgs::NavSatFix> GPS_Out2;
...
CMRosIF_CMNode_Init (int Argc, char **Argv, char *CMNodeName, struct tInfos *Inf)
{
	...
	strcpy(sbuf, "/fix");
    LOG("  -> Publish '%s'", sbuf);
    CMNode.Topics.Pub.GPS_Out.Pub           = node->advertise<sensor_msgs::NavSatFix>(sbuf, static_cast<uint>(CMNode.Cfg.QueuePub));
    // CMNode.Topics.Pub.GPS_Out.Job           = CMCRJob_Create("gps_out");
    CMNode.Topics.Pub.GPS_Out.Job           = CMCRJob_Create("fix");

    strcpy(sbuf, "/gps_out_wo_noise");
    LOG("  -> Publish '%s'", sbuf);
    CMNode.Topics.Pub.GPS_Out2.Pub           = node->advertise<sensor_msgs::NavSatFix>(sbuf, static_cast<uint>(CMNode.Cfg.QueuePub));
    CMNode.Topics.Pub.GPS_Out2.Job           = CMCRJob_Create("gps_out_wo_noise");
...
CMRosIF_CMNode_TestRun_Start_atBegin (struct tInfos *Inf)
{
    ...
    if (VehicleInfo_Err == 0) {
        ...
        CMNode.Topics.Pub.GPS_Out2.CycleTime     = (int)(1000 / iGetIntOpt(Inf_Vehicle, "Sensor.GNav.UpdRate", 10));
		CMNode.Topics.Pub.GPS_Out2.CycleOffset   = iGetIntOpt(Inf_Vehicle, "Sensor.GNav.nCycleOffset", 0);

        CMNode.Topics.Pub.Imu_Out.CycleTime     = (int)(1000 / iGetIntOpt(Inf_Vehicle, "Sensor.GNav.UpdRate", 10));
		CMNode.Topics.Pub.Imu_Out.CycleOffset   = iGetIntOpt(Inf_Vehicle, "Sensor.GNav.nCycleOffset", 0);
	...
	*pmode = CMNode_Mode_Disabled;
	...
	job       = CMNode.Topics.Pub.GPS_Out2.Job;
    cycletime = CMNode.Topics.Pub.GPS_Out2.CycleTime;
    cycleoff  = CMNode.Topics.Pub.GPS_Out2.CycleOffset;
...
CMRosIF_CMNode_Calc (double dt)
{
	...
	if (SimCore.State == SCState_Simulate) {
		...
		if ((rv = CMCRJob_DoPrep(CMNode.Topics.Pub.Lidar_OS1.Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) < CMCRJob_RV_OK) {
            ...
            pc1.header.stamp.sec = wtime.sec;
            pc1.header.stamp.nsec = wtime.nsec;
        ...
        if ((rv = CMCRJob_DoPrep(CMNode.Topics.Pub.GPS_Out2.Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) < CMCRJob_RV_OK) {
			LogErrF(EC_Sim, "cycleTime: %d, cycleoffset: %d, cycle: %lu", CMNode.Topics.Pub.GPS_Out2.CycleTime, CMNode.Topics.Pub.GPS_Out2.CycleOffset, CMNode.CycleNoRel);
			LogErrF(EC_Sim, "CMNode: Error on DoPrep for Job '%s'! rv=%s", CMCRJob_GetName(CMNode.Topics.Pub.GPS_Out2.Job), CMCRJob_RVStr(rv));
		} else if (Lidar_CycleCount % (int)CMNode.Topics.Pub.GPS_Out2.CycleTime == 0) {
			// CMNode.Topics.Pub.GPS_Out2.Msg.cycleno      = CMNode.CycleNoRel;
			// CMNode.Topics.Pub.GPS_Out2.Msg.time         = ros::Time(SimCore.Time);
			// CMNode.Topics.Pub.GPS_Out2.Msg.synthdelay   = CMNode.Sync.SynthDelay;
			
			// ros::WallTime wtime = ros::WallTime::now();
			// CMNode.Topics.Pub.GPS_Out2.Msg.header.stamp.sec  = wtime.sec;
			// CMNode.Topics.Pub.GPS_Out2.Msg.header.stamp.nsec = wtime.nsec;
			
			// // unsigned int noise_deg = 150;
			// double pi = 3.1415926536897932384626433832795028841971;
			// CMNode.Topics.Pub.GPS_Out2.Msg.latitude          = GNavSensor.Receiver.UserPosLlhTsa[0] * 180 / pi; // + ((double)(rand() % (noise_deg * 2 + 1)) - noise_deg) / 10000000;
			// CMNode.Topics.Pub.GPS_Out2.Msg.longitude         = GNavSensor.Receiver.UserPosLlhTsa[1] * 180 / pi; // + ((double)(rand() % (noise_deg * 2 + 1)) - noise_deg) / 10000000;
			// CMNode.Topics.Pub.GPS_Out2.Msg.altitude          = GNavSensor.Receiver.UserPosLlhTsa[2];
		...
		} else if (Lidar_CycleCount % (int)CMNode.Topics.Pub.Imu_Out.CycleTime == 0) {
            tf2::Quaternion q;

			ros::WallTime wtime = ros::WallTime::now();
			CMNode.Topics.Pub.Imu_Out.Msg.header.stamp.sec  = wtime.sec;
			CMNode.Topics.Pub.Imu_Out.Msg.header.stamp.nsec = wtime.nsec;
            CMNode.Topics.Pub.Imu_Out.Msg.header.frame_id = "Fr1A";

            q.setRPY(Vehicle.Roll, Vehicle.Pitch, Vehicle.Yaw);

			CMNode.Topics.Pub.Imu_Out.Msg.orientation = tf2::toMsg(q);
            // .x = q.x;
            // CMNode.Topics.Pub.Imu_Out.Msg.orientation.y = q.y;
            // CMNode.Topics.Pub.Imu_Out.Msg.orientation.z = q.z;
            // CMNode.Topics.Pub.Imu_Out.Msg.orientation.w = q.w;

            CMNode.Topics.Pub.Imu_Out.Msg.angular_velocity.x = Vehicle.RollVel;
            CMNode.Topics.Pub.Imu_Out.Msg.angular_velocity.y = Vehicle.PitchVel;
            CMNode.Topics.Pub.Imu_Out.Msg.angular_velocity.z = Vehicle.YawRate;

            CMNode.Topics.Pub.Imu_Out.Msg.linear_acceleration.x = Vehicle.RollAcc;
            CMNode.Topics.Pub.Imu_Out.Msg.linear_acceleration.y = Vehicle.PitchAcc;
            CMNode.Topics.Pub.Imu_Out.Msg.linear_acceleration.z = Vehicle.YawAcc;
        ...
        if ((rv = CMCRJob_DoPrep(CMNode.Topics.Pub.Vehicle_Out.Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) < CMCRJob_RV_OK) {
            ...
            ros::WallTime wtime = ros::WallTime::now();
            CMNode.Topics.Pub.Vehicle_Out.Msg.header.stamp.sec = wtime.sec;
            CMNode.Topics.Pub.Vehicle_Out.Msg.header.stamp.nsec = wtime.nsec;
...
CMRosIF_CMNode_Out (void)
{
    ...
    auto out_gps_out2 = &CMNode.Topics.Pub.GPS_Out2;
	
    /* Communicate to External ROS Node in this cycle?
     * - The job mechanism is optional and can be e.g. replaced by simple modulo on current cycle
     */
    if ((rv = CMCRJob_DoJob(out_gps_out2->Job, CMNode.CycleNoRel, 1, nullptr, nullptr)) != CMCRJob_RV_DoNothing
            && rv != CMCRJob_RV_DoSomething) {
        LogErrF(EC_Sim, "CMNode: Error on DoJob for Job '%s'! rv=%s",CMCRJob_GetName(out_gps_out2->Job), CMCRJob_RVStr(rv));
    } else if (rv == CMCRJob_RV_DoSomething) {
	
	out_gps_out2->Pub.publish(out_gps_out2->Msg);
