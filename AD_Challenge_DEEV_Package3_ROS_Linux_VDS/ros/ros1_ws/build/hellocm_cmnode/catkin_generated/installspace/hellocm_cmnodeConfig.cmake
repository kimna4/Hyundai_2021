...
if(hellocm_cmnode_CONFIG_INCLUDED)
  return()
endif()
set(hellocm_cmnode_CONFIG_INCLUDED TRUE)

# set variables for source/devel/install prefixes
if("FALSE" STREQUAL "TRUE")
  set(hellocm_cmnode_SOURCE_PREFIX /media/ssd1/2021_Hyundai/Challenge_Model_wROS/AD_Challenge_DEEV_Package3_ROS_Linux_VDS/ros/ros1_ws/src/hellocm_cmnode)
  set(hellocm_cmnode_DEVEL_PREFIX /media/ssd1/2021_Hyundai/Challenge_Model_wROS/AD_Challenge_DEEV_Package3_ROS_Linux_VDS/ros/ros1_ws/devel)
  set(hellocm_cmnode_INSTALL_PREFIX "")
  set(hellocm_cmnode_PREFIX ${hellocm_cmnode_DEVEL_PREFIX})
else()
  set(hellocm_cmnode_SOURCE_PREFIX "")
  set(hellocm_cmnode_DEVEL_PREFIX "")
  set(hellocm_cmnode_INSTALL_PREFIX /media/ssd1/2021_Hyundai/Challenge_Model_wROS/AD_Challenge_DEEV_Package3_ROS_Linux_VDS/ros/ros1_ws/install)
  set(hellocm_cmnode_PREFIX ${hellocm_cmnode_INSTALL_PREFIX})
endif()

# warn when using a deprecated package
if(NOT "" STREQUAL "")
  set(_msg "WARNING: package 'hellocm_cmnode' is deprecated")
...
set(hellocm_cmnode_FOUND_CATKIN_PROJECT TRUE)

if(NOT " " STREQUAL " ")
  set(hellocm_cmnode_INCLUDE_DIRS "")
  set(_include_dirs "")
  if(NOT " " STREQUAL " ")
  ...
  foreach(idir ${_include_dirs})
    ...
    elseif("${idir} " STREQUAL "include ")
      get_filename_component(include "${hellocm_cmnode_DIR}/../../../include" ABSOLUTE)
      if(NOT IS_DIRECTORY ${include})
        message(FATAL_ERROR "Project 'hellocm_cmnode' specifies '${idir}' as an include dir, which is not found.  It does not exist in '${include}'.  ${_report}")
      endif()
    else()
      message(FATAL_ERROR "Project 'hellocm_cmnode' specifies '${idir}' as an include dir, which is not found.  It does neither exist as an absolute directory nor in '\${prefix}/${idir}'.  ${_report}")
    endif()
    _list_append_unique(hellocm_cmnode_INCLUDE_DIRS ${include})
  endforeach()
endif()

set(libraries "CMNode_ROS1_HelloCM")
foreach(library ${libraries})
  # keep build configuration keywords, target names and absolute libraries as-is
  if("${library}" MATCHES "^(debug|optimized|general)$")
    list(APPEND hellocm_cmnode_LIBRARIES ${library})
  elseif(${library} MATCHES "^-l")
    list(APPEND hellocm_cmnode_LIBRARIES ${library})
  elseif(${library} MATCHES "^-")
    # This is a linker flag/option (like -pthread)
    # There's no standard variable for these, so create an interface library to hold it
    if(NOT hellocm_cmnode_NUM_DUMMY_TARGETS)
      set(hellocm_cmnode_NUM_DUMMY_TARGETS 0)
    endif()
    # Make sure the target name is unique
    set(interface_target_name "catkin::hellocm_cmnode::wrapped-linker-option${hellocm_cmnode_NUM_DUMMY_TARGETS}")
    while(TARGET "${interface_target_name}")
      math(EXPR hellocm_cmnode_NUM_DUMMY_TARGETS "${hellocm_cmnode_NUM_DUMMY_TARGETS}+1")
      set(interface_target_name "catkin::hellocm_cmnode::wrapped-linker-option${hellocm_cmnode_NUM_DUMMY_TARGETS}")
    endwhile()
    ...
    list(APPEND hellocm_cmnode_LIBRARIES "${interface_target_name}")
  ...
  elseif(IS_ABSOLUTE ${library})
    ...
    endforeach()
    if(lib)
      _list_append_unique(hellocm_cmnode_LIBRARY_DIRS ${lib_path})
      list(APPEND hellocm_cmnode_LIBRARIES ${lib})
    else()
      # as a fall back for non-catkin libraries try to search globally
      find_library(lib ${library})
      if(NOT lib)
        message(FATAL_ERROR "Project '${PROJECT_NAME}' tried to find library '${library}'.  The library is neither a target nor built/installed properly.  Did you compile project 'hellocm_cmnode'?  Did you find_package() it before the subdirectory containing its code is included?")
      endif()
      list(APPEND hellocm_cmnode_LIBRARIES ${lib})
    endif()
  endif()
...
set(hellocm_cmnode_EXPORTED_TARGETS "")
# create dummy targets for exported code generation targets to make life of users easier
foreach(t ${hellocm_cmnode_EXPORTED_TARGETS})
  if(NOT TARGET ${t})
    add_custom_target(${t})
  endif()
endforeach()

set(depends "message_runtime;cmrosutils;hellocm_msgs")
foreach(depend ${depends})
  string(REPLACE " " ";" depend_list ${depend})
  # the package name of the dependency must be kept in a unique variable so that it is not overwritten in recursive calls
  list(GET depend_list 0 hellocm_cmnode_dep)
  list(LENGTH depend_list count)
  if(${count} EQUAL 1)
    # simple dependencies must only be find_package()-ed once
    if(NOT ${hellocm_cmnode_dep}_FOUND)
      find_package(${hellocm_cmnode_dep} REQUIRED NO_MODULE)
    endif()
  else()
    # dependencies with components must be find_package()-ed again
    list(REMOVE_AT depend_list 0)
    find_package(${hellocm_cmnode_dep} REQUIRED NO_MODULE ${depend_list})
  endif()
  _list_append_unique(hellocm_cmnode_INCLUDE_DIRS ${${hellocm_cmnode_dep}_INCLUDE_DIRS})

  # merge build configuration keywords with library names to correctly deduplicate
  _pack_libraries_with_build_configuration(hellocm_cmnode_LIBRARIES ${hellocm_cmnode_LIBRARIES})
  _pack_libraries_with_build_configuration(_libraries ${${hellocm_cmnode_dep}_LIBRARIES})
  _list_append_deduplicate(hellocm_cmnode_LIBRARIES ${_libraries})
  # undo build configuration keyword merging after deduplication
  _unpack_libraries_with_build_configuration(hellocm_cmnode_LIBRARIES ${hellocm_cmnode_LIBRARIES})

  _list_append_unique(hellocm_cmnode_LIBRARY_DIRS ${${hellocm_cmnode_dep}_LIBRARY_DIRS})
  list(APPEND hellocm_cmnode_EXPORTED_TARGETS ${${hellocm_cmnode_dep}_EXPORTED_TARGETS})
endforeach()

set(pkg_cfg_extras "")
foreach(extra ${pkg_cfg_extras})
  if(NOT IS_ABSOLUTE ${extra})
    set(extra ${hellocm_cmnode_DIR}/${extra})
  endif()
  include(${extra})
endforeach()
